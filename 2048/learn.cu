...
const int recordSize=50000;
const int attrNumLimit=4;
const int attrNum=4;
const int attrSlotNum=4;
const int attrDataSize=16;
const int branchSizeLimit=1000;
const int branchSize=208;//16*13;
...
__global__ void run_d(float *attr_d,int *attrPosition_d, record_d *rec_d, curandState_t *randState, float learnSpeed, int *learnCnt_d, int *recProgress_d)
{
	...
	do{
		...
		__syncthreads();
		if(tar_ds==-1) break;
		if(wayID==0 && directID==0 && attrID<4){
			if( learn ) {
...
double run(vector<Attr> &attr, int times, float learnSpeed = 0, int *learnCnt_d = NULL, int *recProgress_d = NULL){
	...
	*/
	...
	/*
	for(int T=0; T<times/branchSize; T++){
		run_d<<<branchSize,(8*4)*attr.size()>>>(attr_d,attrPosition_d,rec_d,randState,learnSpeed,learnCnt_d,recProgress_d);
	}
	run_d<<<times%branchSize,(8*4)*attr.size()>>>(attr_d,attrPosition_d,rec_d,randState,learnSpeed,learnCnt_d,recProgress_d);
	*/
	for(int i=0; i<attr.size(); i++){
